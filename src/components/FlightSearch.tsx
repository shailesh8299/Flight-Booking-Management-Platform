...
const FlightSearch = () => {
  ...
  const [returnDate, setReturnDate] = useState(format(new Date(Date.now() + 86400000 * 3), 'yyyy-MM-dd'));
  const [passengers, setPassengers] = useState(1);

  // Multi-city legs
  const [legs, setLegs] = useState([
    { from: 'New Delhi (DEL)', to: 'Mumbai (BOM)', date: format(new Date(), 'yyyy-MM-dd') },
